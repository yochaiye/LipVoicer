...
from ASR.nnet import collate_fn
from ASR.nnet import transforms
from ASR.nnet import collate_fn
...
class LRS(Dataset):
    ...
    def __getitem__(self, n):
        ...
        if self.load_audio:
            ...
            if 'trainval' in self.paths[n]:
                audio_path = self.paths[n].replace('trainval', 'audio_only/trainval')
            elif 'test' in self.paths[n]:
                audio_path = self.paths[n].replace('test', 'audio_only/test')
            audio = torch.load(audio_path+'.wav.spec')
        else:
            audio = None
        ...
        if self.load_audio:
            ...
            audio = ((audio - min_val) / (-min_val / 2)) - 1
