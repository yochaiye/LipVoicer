...
from ASR.nnet.losses import loss_dict
from ASR.nnet.decoders import decoder_dict
from ASR.nnet.metrics import metric_dict

# Neural Nets
from ASR.nnet.module import Module
from ASR.nnet import normalizations as norms
...
class Model(Module):
    def __init__(self, name="model", T=400, beta_0=0.0001, beta_T=0.02):
    ...
    def distribute_strategy(self, rank, sync_batch_norm=True):
        if sync_batch_norm:
            ...
            object.__setattr__(
                self,
                "ddp",
                torch.nn.parallel.DistributedDataParallel(self, device_ids=[rank]),
            )
        self.rank = rank
    ...
    def compile(
        self, losses, loss_weights=None, optimizer="Adam", metrics=None, decoders=None
    ):
        # Optimizer
    ...
    def map_to_outputs(self, outputs, struct):
        ...
        # Dictionary
            ...
            for key in struct:
                if not key in outputs:
                    raise Exception(
                        "Found unexpected dict key: {}. Valid output names are: {}".format(
                            key, outputs.keys()
                        )
                    )
        ...
        # List
            ...
            struct = {
                key: struct[i] if i < len(struct) else None
    ...
    def forward_model(self, inputs, diffusion_steps, targets, compute_metrics=True, verbose=0):
        ...
        if self.is_distributed:
            outputs = self.ddp(inputs, diffusion_steps)
        ...
        for key in outputs:
            ...
            if self.losses[key] != None:
                ...
                # Weight Loss
                total_loss = total_loss + batch_losses[key_loss] * self.loss_weights[key].get_val_step(self.model_step + 1)
                ...
                for metric, decoder in zip(metrics, decoders):
                    ...
                    if decoder != None:
                        batch_truths[key_metric] = (
                            decoder(targets[key], from_logits=False)
                            if targets[key] != None
                            else None
                        )
                    ...
                    # Metric
                    batch_metrics[key_metric] = metric(
                        batch_truths[key_metric], batch_preds[key_metric]
                    )
    ...
    def train_step(self, inputs, targets, precision, grad_scaler, accumulated_steps, acc_step, eval_training):
        ...
        if "cuda" in str(self.device):
            with torch.cuda.amp.autocast(enabled=precision != torch.float32, dtype=precision):
                batch_losses, batch_metrics, batch_truths, batch_preds = self.forward_model(inputs, diffusion_steps, targets, compute_metrics=eval_training)
        else:
            batch_losses, batch_metrics, batch_truths, batch_preds = self.forward_model(inputs, diffusion_steps, targets, compute_metrics=eval_training)
        ...
        if "cuda" in str(self.device) and self.model_step % 100 == 0:
            self.add_info(
                "{}_memory".format(str(self.device)),
                round(
                    100
        ...
        if self.ema_model != None:
            for param_target, param_net in zip(
                self.ema_model.parameters(), self.parameters()
            ):
                param_target.mul_(self.ema_tau)
                param_target.add_((1 - self.ema_tau) * param_net.detach())
            for buffer_target, buffer_net in zip(
                self.ema_model.buffers(), self.buffers()
            ):
                buffer_target.copy_(buffer_net)

        return batch_losses, batch_metrics, acc_step

    def eval_step(self, inputs, diffusion_steps, targets, verbose=0):
        with torch.no_grad():
            batch_losses, batch_metrics, batch_truths, batch_preds = self.forward_model(inputs, diffusion_steps, targets, verbose=verbose)
        ...
        for id, (key, value) in enumerate(state_dict.items()):
            print(
                "{} {} type: {:<12} numel: {:<12} shape: {:<20} mean: {:<12.4f} std: {:<12.4f} min: {:<12.4f} max: {:<12.4f} dtype: {:<12} device: {}".format(
                    str(id) + " " * (max_len_id - len(str(id))),
                    key + " " * (max_len_key - len(key)),
                    "param" if isinstance(value, nn.Parameter) else "buffer",
                    value.numel(),
    ...
    def show_modules(self, module=None):
        ...
        max_len_class = (
            max([len(type(value).__name__) for value in named_modules.values()]) + 5
        )
    ...
    def plot_params(self):
        for param_name, param in self.named_parameters():
            plt.title(
                "{} \n shape: {} / mean: {:.4f} / std: {:.4f}".format(
                    param_name, str(tuple(param.size())), param.mean(), param.std()
                )
            )
        ...
        torch.save(
            {
                ...
                else {key: value.state_dict() for key, value in self.optimizer.items()}
    ...
    def on_step_end(
        ...
        if save and callback_path and self.rank == 0:
            self.save(
                os.path.join(
                    callback_path,
                    "checkpoints_epoch_{}_step_{}.ckpt".format(epoch, self.model_step),
                )
            )
        ...
        try:
            ...
            for epoch in range(initial_epoch, epochs):
                ...
                for step, batch in enumerate(epoch_iterator):
                    ...
                    batch_losses, batch_metrics, acc_step = self.train_step(
                        inputs=inputs,
                        targets=targets,
                        precision=precision,
                    ...
                    for key, value in batch_losses.items():
                        epoch_losses[key] = (
                            ...
                            else value.detach().type(torch.float64)
                        )
                ...
                if self.is_distributed:
                    epoch_losses, epoch_metrics = self.reduce_losses_metrics(
        ...
        # Exception Handler
            ...
            if self.rank == 0 and writer is not None:
                writer.add_text(
                    "Exceptions",
                    "Rank: {} \nDate: {} \n{}".format(
                        str(self.rank), time.ctime(), str(e)
                    ),
                    self.model_step,
                )
    ...
    def swa(
        ...
        for epoch in epochs_list:
            ...
            self.load(
                glob.glob(
                    os.path.join(
                        callback_path, "checkpoints_epoch_{}_step_*.ckpt".format(epoch)
                    )
                )[0]
            )
        ...
        while steps < update_steps:
            for step, batch in enumerate(dataset):
                ...
                with torch.cuda.amp.autocast(
                    enabled=precision != torch.float32, dtype=precision
                ):
                    with torch.no_grad():
                        self.forward(inputs)
        ...
        if self.rank == 0:
            self.save(
                os.path.join(
                    callback_path,
                    "checkpoints_swa-{}-{}-{}.ckpt".format(
                        swa_type, epochs_list[0], epochs_list[-1]
                    ),
                ),
    ...
    def generate(self, dataset, saving_path=None):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # Generate Samples
            self.forward_generate(
                inputs, saving_path, "sample_" + str(self.rank) + "_" + str(ctr)
            )
            ctr += 1
