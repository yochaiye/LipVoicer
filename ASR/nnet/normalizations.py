...
from ASR.nnet import initializations
...
class BatchNorm1d(nn.BatchNorm1d):
    ...
    def __init__(
        ...
        if weight_init != "default" and self.affine:
            if isinstance(weight_init, dict):
                initializations.init_dict[weight_init["class"]](
                    self.weight, **weight_init["params"]
                )
            else:
                initializations.init_dict[weight_init](self.weight)
        if bias_init != "default" and self.affine:
            if isinstance(bias_init, dict):
                initializations.init_dict[bias_init["class"]](
                    self.bias, **bias_init["params"]
                )
            else:
    ...
    def forward(self, input):
        ...
        output = self.output_permute(
            super(BatchNorm1d, self).forward(self.input_permute(input))
        )
...
class BatchNorm2d(nn.BatchNorm2d):
    ...
    def forward(self, input):
        ...
        output = self.output_permute(
            super(BatchNorm2d, self).forward(self.input_permute(input))
        )
...
class BatchNorm3d(nn.BatchNorm3d):
    ...
    def forward(self, input):
        ...
        output = self.output_permute(
            super(BatchNorm3d, self).forward(self.input_permute(input))
        )
...
class SyncBatchNorm(nn.SyncBatchNorm):
    ...
    def forward(self, input):
        ...
        output = self.output_permute(
            super(SyncBatchNorm, self).forward(self.input_permute(input))
        )
...
class InstanceNorm2d(nn.InstanceNorm2d):
    ...
    def forward(self, input):
        return self.output_permute(
            super(InstanceNorm2d, self).forward(self.input_permute(input))
        )
...
class InstanceNorm3d(nn.InstanceNorm3d):
    ...
    def forward(self, input):
        return self.output_permute(
            super(InstanceNorm3d, self).forward(self.input_permute(input))
        )
...
class GroupNorm(nn.GroupNorm):
    ...
    def forward(self, input):
        return self.output_permute(
            super(GroupNorm, self).forward(self.input_permute(input).contiguous())
        )
