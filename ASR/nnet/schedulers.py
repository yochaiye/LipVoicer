...
class LinearDecayScheduler(Scheduler):
    ...
    def get_val_step(self, step):
        ...
        if step >= self.decay_steps:
            ...
            val = (
                self.value_start
                - step * (self.value_start - self.value_end) / self.decay_steps
            )
...
class ExpDecayScheduler(Scheduler):
    ...
    def get_val_step(self, step):
        ...
        arg2 = self.val_max * self.alpha ** (
            (step - self.warmup_steps) / (self.end_step - self.warmup_steps)
        )  # Decay phase
        val = min(arg1, arg2)
...
class CosineAnnealingScheduler(Scheduler):
    ...
    def get_val_step(self, step):
        ...
        if step <= self.warmup_steps:  # Warmup phase
            ...
            val = (self.val_max - self.val_min) * 0.5 * (
                1
                + math.cos(
                    math.pi
