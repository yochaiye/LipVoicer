...
# NeuralNets
from ASR.nnet import embeddings
from ASR.nnet import schedulers
...
def get_decay_param_groups(
    ...
    for module_name, module in model.named_modules():
        for param_name, param in module.named_parameters():
            # Full Param Name
            full_param_name = (
                "{}.{}".format(module_name, param_name) if module_name else param_name
            )
            ...
            for decay_param in decay_params:
                # Decay Weight
                if param_name.endswith(decay_param) and isinstance(
                    module, decay_modules
                ):
    ...
    assert (
        len(inter_params) == 0
    ), "parameters {} made it into both decay/no_decay sets!".format(str(inter_params))
    assert (
...
optim_dict = {"SGD": SGD, "RMSprop": optim.RMSprop, "Adam": Adam, "AdamW": AdamW}
