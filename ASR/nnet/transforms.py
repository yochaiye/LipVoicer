    ...
    def forward(self, x):
        ...
        for _ in range(mT):
            x = torchaudio.functional.mask_along_axis(
                x,
                mask_param=self.T,
                mask_value=x.mean() if self.mean_frame else 0.0,
                axis=2,
            )
...
class BabbleNoise(nn.Module):
    def __init__(
        self,
        noise_file_path="datasets/NoiseX/babble/babble.flac",
        SNR_db=[-5, 0, 5, 10, 15, 20, None],
        ...
        if to_sample_rate != None:
            self.noise = torchaudio.functional.resample(
                self.noise, orig_freq=self.from_sample_rate, new_freq=to_sample_rate
            )
        self.SNR_db = SNR_db
...
def align_video_to_audio(video, audio):
    ...
    video = torch.cat(
        [
            video.new_zeros(padding_left, H, W, C),
            video,
            video.new_zeros(padding_right, H, W, C),
        ],
        dim=0,
    )
...
class LipDetectCrop(nn.Module):
    ...
    def warp_img(self, src, dst, img, std_size):
        ...
        warped = (
            warped * 255
        )  # note output from wrap is double image (value range [0,1])
    ...
    def apply_transform(self, transform, img, std_size):
        """apply_transform.
            ...
            """
        warped = skimage.transform.warp(
            img, inverse_map=transform.inverse, output_shape=std_size
        )
        warped = (
            warped * 255
        )  # note output from wrap is double image (value range [0,1])
