...
from ASR.nnet import activations
...
class ConvNeuralNetwork(nn.Module):
    def __init__(
        ...
        conv = {1: layers.Conv1d, 2: layers.Conv2d, 3: layers.Conv3d}
            ...
            [
                nn.Sequential(
                    conv[dim](
                        dim_input if layer_id == 0 else dim_layers[layer_id - 1],
                        dim_layers[layer_id],
                        kernel_size[layer_id]
                        if isinstance(kernel_size, list)
                        else kernel_size,
                        stride=strides[layer_id]
                        if isinstance(strides, list)
                        else strides,
                        padding=padding[layer_id]
                    ...
                    norm(
                        dim_layers[layer_id], **norm_params, channels_last=channels_last
                    ),
                    act_fun(**act_fun_params),
                    nn.Dropout(drop_rate) if drop_rate > 0 else nn.Identity(),
                )
            ...
            [
                nn.Sequential(
                    conv[dim](
                        ...
                        output_padding=output_padding[layer_id]
...
class InceptionModule(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, kernel_sizes=[3, 5], dim=2, inplace=False
    ):
...
class FeedForwardModule(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.Sequential(
            ...
            layers.Linear(
                dim_model, dim_ffn, weight_init=weight_init, bias_init=bias_init
            ),
            activations.act_dict[act_fun](),
            nn.Dropout(p=drop_rate) if inner_dropout else nn.Identity(),
            layers.Linear(
                dim_ffn, dim_model, weight_init=weight_init, bias_init=bias_init
            ),
...
class AttentionModule(nn.Module):
    ...
    def __init__(
        ...
        # Pre Norm
        if isinstance(norm, dict):
            self.norm = normalizations.norm_dict[norm["class"]](
                dim_model, **norm["params"], channels_last=channels_last
            )
        else:
            self.norm = normalizations.norm_dict[norm](
                dim_model, channels_last=channels_last
            )

        # Attention
        self.attention = attentions.att_dict[att_params["class"]](
            dim_model=dim_model, **att_params["params"]
        )
    ...
    def forward(self, x, x_cross=None, mask=None):
        ...
        # Self-Attention
        x = self.attention.forwardQKV(
            Q=x,
            K=x_cross if x_cross != None else x,
            V=x_cross if x_cross != None else x,
            mask=mask,
        )
...
class ConvolutionModule(nn.Module):
    ...
    def __init__(
        ...
        # Layers
        pointwise_conv = layers.layer_dict[
            conv_params["class"].replace("Transpose", "")
        ]
        depthwise_conv = layers.layer_dict[conv_params["class"]]
        if batch_norm:
            norm = normalizations.norm_dict[
                conv_params["class"]
                .replace("Transpose", "")
                .replace("Conv", "BatchNorm")
            ]
        else:
        ...
        self.layers = nn.Sequential(
            ...
            pointwise_conv(
                dim_model, 2 * dim_expand, kernel_size=1, channels_last=channels_last
            ),
            ...
            activations.act_dict[act_fun](),
            pointwise_conv(
                dim_expand, dim_expand, kernel_size=1, channels_last=channels_last
            ),
            nn.Dropout(p=drop_rate),
        )
...
class FusionModule(nn.Module):
    def __init__(self, a_dim_model=360, v_dim_model=360, f_dim_model=360, ff_ratio=4):
        ...
        self.layers = nn.Sequential(
            layers.Linear(
                dim_in, dim_ffn, weight_init=weight_init, bias_init=bias_init
            ),
            activations.act_dict[act_fun](),
            layers.Linear(
                dim_ffn, dim_out, weight_init=weight_init, bias_init=bias_init
            ),
        )
