...
class ResNetBlock(nn.Module):
    ...
    def __init__(
        ...
        # Joined Post Act
        self.joined_post_act = (
            act_fun(**act_fun_params) if joined_post_act else nn.Identity()
        )
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        # Post Norm
        self.post_norm = (
            nn.LayerNorm(normalized_shape=dim_model) if post_norm else nn.Identity()
        )
...
class ConformerBlock(nn.Module):
    def __init__(
        ...
        if dim_model != dim_expand:  # Pointwise Conv Residual
            if transposed_block:
                self.conv_res = layers.layer_dict[conv_params["class"]](
                    dim_model,
                    dim_expand,
                    kernel_size=1,
                    stride=conv_stride,
                    channels_last=True,
                    output_padding=conv_params["params"].get("output_padding", 0),
                )
            else:
                self.conv_res = layers.layer_dict[conv_params["class"]](
                    dim_model,
            ...
            if transposed_block:
                self.conv_res = layers.layer_dict[
                    conv_params["class"].replace("ConvTranspose", "Upsample")
                ](scale_factor=conv_stride)
            else:
                self.conv_res = layers.layer_dict[
                    conv_params["class"].replace("Conv", "MaxPool")
                ](kernel_size=1, stride=conv_stride)
...
block_dict = {"ConformerBlock": ConformerBlock}
