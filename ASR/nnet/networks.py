...
class Transformer(nn.Module):
    def __init__(
        ...
        # LayerNorm
        self.layernorm = (
            nn.LayerNorm(normalized_shape=dim_model) if not post_norm else nn.Identity()
        )
    ...
    def __init__(self, include_head=True, vocab_size=256, att_type="patch", interctc_blocks=[3, 6, 10, 13], num_blocks=[5, 6, 5], loss_prefix="ctc"):
        ...
        self.back_end = ConformerInterCTC(
            ...
            if att_type == "regular"
                ...
                {
                    "class": "GroupedRelPosMultiHeadSelfAttention",
                    "params": {
                        "num_heads": num_heads,
                        "group_size": 3,
                        "attn_drop_rate": attn_drop_rate,
...
class VisualEfficientConformerEncoder(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        x = (
            x.squeeze(dim=-1).squeeze(dim=-1).transpose(1, 2)
        )  # (B, C, T, 1, 1) -> (B, T, C)
...
class AudioVisualEfficientConformerEncoder(nn.Module):
    def __init__(
        ...
        # Visual Encoder
        self.video_encoder = VisualEfficientConformerEncoder(
            include_head=False,
            vocab_size=vocab_size,
            interctc_blocks=v_interctc_blocks,
            num_blocks=v_num_blocks,
            loss_prefix="v_ctc",
        )

        # Audio Encoder
        self.audio_encoder = AudioEfficientConformerEncoder(
            include_head=False,
            vocab_size=vocab_size,
        ...
        # Fusion Module
        self.fusion_module = modules.FusionModule(
            a_dim_model=dim_model, v_dim_model=dim_model, f_dim_model=dim_model
        )
        ...
        # Head
        self.head = (
            layers.Linear(dim_model, vocab_size) if include_head else nn.Identity()
        )
