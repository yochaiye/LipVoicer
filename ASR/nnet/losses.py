...
class HingeLoss(nn.Module):
    ...
    def forward(self, targets, outputs):
        ...
        # Compute Loss
        loss = torch.where(
            y > 0, (self.margin - y_pred).relu(), (self.margin + y_pred).relu()
        )
...
class MeanAbsoluteError(nn.Module):
    ...
    def __init__(
        self, convert_one_hot=False, one_hot_axis=-1, masked=False, reduction="mean"
    ):
    ...
    def forward(self, targets, outputs):
        ...
        if self.convert_one_hot:
            y = F.one_hot(y, num_classes=y_pred.size(self.one_hot_axis)).type(
                y_pred.dtype
            )
...
class MeanSquaredError(nn.Module):
    ...
    def __init__(
        self, convert_one_hot=False, axis=-1, targets=None, factor=1.0, reduction="mean"
    ):
...
loss_dict = {"SoftmaxCrossEntropy": SoftmaxCrossEntropy, "CTC": CTCLoss}
