...
# Initializations
from ASR.nnet.initializations import init_dict
...
class Conv1d(nn.Conv1d):
    def __init__(
        ...
        if padding == "valid":
            ...
            padding == "same-left"
        ):  # Prioritize left context rather than right for even kernels, cause strided convolution with even kernel to be asymmetric!
            self.pre_padding = nn.ConstantPad1d(
            ...
            self.pre_padding = nn.ConstantPad1d(
                padding=(self.kernel_size[0] - 1, 0), value=0
            )
    ...
    def __init__(
        ...
        if mask == "A":
            ...
            mask = torch.cat(
                [
                    torch.ones(
                        torch.div(stem_kernel_numel, 2, rounding_mode="floor"),
                        dtype=torch.float32,
                    ),
                    torch.zeros(
                        torch.div(stem_kernel_numel + 1, 2, rounding_mode="floor"),
                        dtype=torch.float32,
                    ),
                ],
                dim=0,
            ).reshape(self.kernel_size)
...
class ConvTranspose1d(nn.ConvTranspose1d):
    ...
    def forward(self, x):
        # Apply Weight
        x = self.output_permute(
            super(ConvTranspose1d, self).forward(self.input_permute(x))
        )
...
class ConvTranspose2d(nn.ConvTranspose2d):
    ...
    def forward(self, x):
        # Apply Weight
        x = self.output_permute(
            super(ConvTranspose2d, self).forward(self.input_permute(x))
        )
...
class ConvTranspose3d(nn.ConvTranspose3d):
    def __init__(
        ...
        super(ConvTranspose3d, self).__init__(
            ...
            kernel_size=kernel_size,
            stride=stride,
            padding=tuple([k - 1 for k in _triple(kernel_size)])
    ...
    def forward(self, x):
        return self.output_permute(
            super(ConvTranspose3d, self).forward(
                self.input_permute(self.pre_padding(x))
            )
        )
...
class AvgPool1d(nn.AvgPool1d):
    ...
    def forward(self, x):
        return self.output_permute(
            super(AvgPool1d, self).forward(self.input_permute(x))
        )
...
class AvgPool2d(nn.AvgPool2d):
    ...
    def forward(self, x):
        return self.output_permute(
            super(AvgPool2d, self).forward(self.input_permute(x))
        )
...
class AvgPool3d(nn.AvgPool3d):
    ...
    def forward(self, x):
        return self.output_permute(
            super(AvgPool3d, self).forward(self.input_permute(x))
        )
...
class Embedding(nn.Embedding):
    def __init__(
        self, num_embeddings, embedding_dim, padding_idx=None, weight_init="default"
    ):
...
class GlobalAvgPool1d(nn.Module):
    ...
    def forward(self, x, mask=None):
        if mask != None:
            x = (x * mask).sum(dim=self.dim, keepdim=self.keepdim) / mask.count_nonzero(
                dim=self.dim
            )
        else:
