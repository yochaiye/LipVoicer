...
class Module(nn.Module):
    ...
    def transfer_to_device(self, struct, device=None):
        ...
        if isinstance(struct, dict):
            return {
                key: self.transfer_to_device(value, device=device)
            ...
            return tuple(
                [self.transfer_to_device(value, device=device) for value in struct]
            )
            ...
            raise Exception(
                "Incorrect struct type: {}. Must be dict, list module, tensor or None.".format(
                    type(struct)
                )
            )
