...
def main(rank, args):
    ...
    if args.mode == "training":
        model.fit(
            ...
            step_log_period=args.step_log_period,eval_training=getattr(args.config, "eval_training", not args.no_eval_training),
    ...
    # Stochastic Weight Averaging
        ...
        model.swa(
            ...
            end_epoch=args.swa_epochs[1] if args.swa_epochs else None,
            epochs_list=args.swa_epochs_list,
            update_steps=args.steps_per_epoch,
            swa_type=args.swa_type,
            precision=args.config.precision,
        )
    ...
    # Eval Time
        ...
        eval_time = model.eval_time(
            dataset_eval, eval_steps=getattr(args.config, "eval_steps", args.eval_steps)
        )
        if args.rank == 0:
