...
def load_model(args):
    ...
    if "cuda" in str(device):
        print(
            "Rank {} device: {}, {}, {}MB".format(
                args.rank,
                device,
                torch.cuda.get_device_properties(device).name,
                int(torch.cuda.get_device_properties(device).total_memory // 1e6),
            )
        )
...
def load_datasets(args):
    ...
    if hasattr(args.config, "training_dataset"):
        ...
        if args.rank == 0:
            if args.distributed:
                print(
                    "Training Dataset: {}, {:,} samples - {:,} batches - batch size {} x {}".format(
                        dataset_train.dataset.__class__.__name__,
                        len(dataset_train.dataset),
                        len(dataset_train),
                        dataset_train.dataset.batch_size,
                        args.num_gpus,
                    )
                )
            else:
                print(
                    "Training Dataset: {}, {:,} samples - {:,} batches - batch size {}".format(
                        dataset_train.dataset.__class__.__name__,
                        len(dataset_train.dataset),
                        len(dataset_train),
                        dataset_train.dataset.batch_size,
    ...
    if hasattr(args.config, "evaluation_dataset"):
        ...
        if isinstance(args.config.evaluation_dataset, list):
            ...
            for dataset in args.config.evaluation_dataset:
                ...
                if args.rank == 0:
                    if args.distributed:
                        print(
                            "Evaluation Dataset: {}, {:,} samples - {:,} batches - batch size {} x {}".format(
                                dataset_eval[-1].dataset.__class__.__name__,
                                len(dataset_eval[-1].dataset),
                                len(dataset_eval[-1]),
                                dataset_eval[-1].dataset.batch_size,
                                args.num_gpus,
                            )
                        )
                    else:
                        print(
                            "Evaluation Dataset: {}, {:,} samples - {:,} batches - batch size {}".format(
                                dataset_eval[-1].dataset.__class__.__name__,
                                len(dataset_eval[-1].dataset),
                                len(dataset_eval[-1]),
                                dataset_eval[-1].dataset.batch_size,
        ...
        # One Evaluation dataset
            ...
            dataset_eval = torch.utils.data.DataLoader(
                ...
                shuffle=False
                if args.distributed
                else args.config.evaluation_dataset.shuffle,
                sampler=torch.utils.data.distributed.DistributedSampler(
                    args.config.evaluation_dataset,
                    num_replicas=args.world_size,
                    rank=args.rank,
            ...
            if args.rank == 0:
                if args.distributed:
                    print(
                        "Evaluation Dataset: {}, {:,} samples - {:,} batches - batch size {} x {}".format(
                            dataset_eval.dataset.__class__.__name__,
                            len(dataset_eval.dataset),
                            len(dataset_eval),
